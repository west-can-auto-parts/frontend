...
const page = () => {
    ...
    // Replace hyphen after "Parts" or "(Warehouse)" with a plus
    if (str.includes("(Warehouse)-")) {
        str = str.replace("(Warehouse)-", "(Warehouse)+");
    } else if (str.includes("Parts-")) {
        str = str.replace("Parts-", "Parts+");
    }
    return str.trim();
}
    const locations = [
            ...
            <section className=''>
                ...
                <div className="flex flex-wrap md:flex-nowrap gap-8 w-10/12 mx-auto items-start">
                    <div className="w-full md:w-3/4 grid grid-cols-1 md:grid-cols-2 gap-8">
                        {locations.map(blog => (
                            <div key={blog.id} className="mb-6 h-full group" onClick={() => router.push(`/store/${stringToSlug(blog.name)}`)}>
                                <div className='bg-white shadow-md rounded-lg flex flex-col h-full'>
                                    ...
                                    <div className='p-4 flex flex-1 flex-col'>
                                        ...
                                        <Link href={`/store/${stringToSlug(blog.name)}`}>
